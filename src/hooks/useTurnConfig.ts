 ...
 */
const getIceServersFromPayload = (payload: unknown): RTCIceServer[] => {
  if (isRTCIceServer(payload)) return [payload]

  const candidates = Array.isArray(payload)
    ? payload
    : (payload as { iceServers?: unknown })?.iceServers

  return Array.isArray(candidates) ? candidates.filter(isRTCIceServer) : []
}
...
const fetchTurnServer = async (): Promise<RTCIceServer[]> => {
  ...
  try {
    ...
    const iceServers = getIceServersFromPayload(await response.json())

    if (!iceServers.length) {
      throw new Error(
        'Invalid TURN server response: no valid RTCIceServer entries'
      )
    }
...
export const useTurnConfig = (
  ...
  const turnConfig = useMemo((): RTCConfiguration => {
    const iceServers: RTCIceServer[] = [...(rtcConfig.iceServers ?? [])]

    // Only include TURN server if we have one from API
    if (isEnhancedConnectivityAvailable && enableApiRequest && turnServers) {
